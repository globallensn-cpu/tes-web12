  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          title="Auto-Routing Fallback"
          value="13 Model"
          subtext="Cascading chain pro -> flash -> lite"
          badge={{ text: 'Anti-Limit Engine', type: 'info' }}
      ...
      <div className="flex items-center justify-between border-b border-slate-200 pb-2">
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span>Daftar API Key & Limit Kuota ({safeKeys.length})</span>
          </button>

          <button
            type="button"
            onClick={() => setActiveTab('logs')}
            className={`px-4 py-2 rounded-xl text-xs font-extrabold transition-all flex items-center gap-1.5 cursor-pointer relative ${
              activeTab === 'logs'
                ? 'bg-[#3525cd] text-white shadow-xs'
                : 'bg-white text-slate-600 hover:bg-slate-100 border border-slate-200'
            }`}
          ...
          >
            ...
            <span>⚡ LLM Gateway Live (99.9% Uptime)</span>
          </button>
        </div>
        ...
        <button
          ...
          className="p-2 rounded-xl border border-slate-200 text-slate-500 hover:text-slate-800 hover:bg-slate-50 text-xs font-bold flex items-center gap-1 cursor-pointer transition-colors"
      ...
      {activeTab === 'keys' && (
        <div className="space-y-4">
          {/* Smart API Routing Status Banner */}
          <div className="p-4 bg-gradient-to-r from-indigo-50/90 via-blue-50/70 to-emerald-50/80 border border-indigo-200/80 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-3 text-xs shadow-2xs">
            <div className="flex items-start sm:items-center gap-3">
              <div className="p-2 rounded-xl bg-[#3525cd] text-white shadow-xs shrink-0">
                <Cpu className="w-4 h-4" />
              </div>
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="font-extrabold text-slate-900 text-sm">
                    API Key Routing Otomatis
                  </span>
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-emerald-100 text-emerald-800 border border-emerald-300 flex items-center gap-1">
                    ...
                    Prioritas Admin Polled Aktif
                  </span>
                </div>
                <p className="text-slate-600 text-xs mt-0.5 leading-relaxed">
                  Gateway AI memprioritaskan <strong>{safeKeys.filter(k => (k.verifiedByAdmin || k.lastPolledAt) && k.status === 'active').length} API Key</strong> yang telah diverifikasi (polled) aktif di dashboard ini sebelum fallback ke key cadangan.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 shrink-0 self-end sm:self-center">
              <button
                ...
                className="px-3 py-1.5 rounded-xl bg-white hover:bg-slate-50 text-[#3525cd] border border-indigo-200 text-xs font-bold transition-all flex items-center gap-1.5 shadow-2xs cursor-pointer disabled:opacity-50"
              >
                <Radio className={`w-3.5 h-3.5 ${isPollingAll ? 'animate-pulse text-indigo-600' : 'text-[#3525cd]'}`} />
                <span>{isPollingAll ? 'Memeriksa Pool...' : 'Uji Ulang Semua Key'}</span>
              </button>
            </div>
          </div>
            ...
            headerActions={
              <div className="flex items-center gap-2">
                <button
                  ...
                  onClick={handlePollAllKeys}
                  disabled={isPollingAll || safeKeys.length === 0}
                  className="px-3 py-2 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-200 text-xs font-bold transition-all flex items-center gap-1.5 shadow-2xs cursor-pointer disabled:opacity-50"
                  title="Poll dan uji seluruh API Key Gemini di dalam pool secara serentak"
                >
                  <Radio className={`w-3.5 h-3.5 ${isPollingAll ? 'animate-pulse text-emerald-600' : 'text-emerald-700'}`} />
            ...
            <form onSubmit={handleAddKey} className="space-y-4">
              ...
              <div className="flex items-center justify-end gap-2 pt-3">
                ...
                <button
                  type="submit"
                  disabled={isSavingKey}
                  className="px-4 py-2 rounded-xl bg-[#3525cd] hover:bg-[#2c1eb3] text-white text-xs font-bold shadow-xs cursor-pointer disabled:opacity-60 flex items-center gap-1.5"
                >
                  {isSavingKey ? (
                    <>
